...
class YOLOXDiscriminator_attention(nn.Module):
    ...
    def forward(self, feature, attention_map=None, domain='source'):
        assert attention_map is not None
        attention_map_s, attention_map_t = attention_map
        ...
        loss_s = torch.bmm((1+attention_map_s), loss_s.squeeze(dim=1)).mean()
        loss_t = self.loss_fn(x_t, target_target)
        loss_t = torch.bmm((1+attention_map_t), loss_t.squeeze(dim=1)).mean()
